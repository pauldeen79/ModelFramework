...
public class ViewModelTemplateInitializer : ITemplateInitializer
{
    ...
    public ViewModelTemplateInitializer(IEnumerable<ITemplateParameterConverter> converters)
    {
    ...
    private static void TrySetViewModelOnTemplate(IRenderTemplateRequest request)
    {
    ...
    private void TrySetAdditionalParametersOnTemplate(IRenderTemplateRequest request)
    {
        ...
        {
            initializeMethod.Invoke(request.Template, new object?[] { null });
        }
        ...
        if (modelProperty is not null && modelProperty.CanWrite)
        {
            modelProperty.SetValue(request.Template, ConvertType(request.Model, modelProperty.PropertyType));
        }

        var modelField = templateType.GetField("_modelField", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
        if (modelField is not null)
        {
            modelField.SetValue(request.Template, request.Model);
        }
        ...
        foreach (var item in session.Where(x => x.Key != "Model"))
        {
