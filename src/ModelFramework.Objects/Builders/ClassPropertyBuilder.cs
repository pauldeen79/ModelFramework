...
namespace ModelFramework.Objects.Builders
{
    public partial class ClassPropertyBuilder
    {
        public ClassPropertyBuilder ConvertCollectionOnBuilderToEnumerable()
            => AddMetadata
            (
                new MetadataBuilder().WithName(MetadataNames.CustomImmutableArgumentType)
                                     .WithValue("System.Collections.Generic.IEnumerable<{1}>"),
                new MetadataBuilder().WithName(MetadataNames.CustomImmutableDefaultValue)
                                     .WithValue("new System.Collections.Generic.List<{1}>({0} ?? new Enumerable.Empty<{1}>())")
            );
        ...
        public ClassPropertyBuilder ConvertSinglePropertyToBuilderOnBuilder(string? argumentType = null,
            ...
            => AddMetadata
            (
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderArgumentType)
                                     .WithValue(argumentType ?? "{0}Builder"),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderMethodParameterExpression)
                                     .WithValue("{0}.Build()"),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderConstructorInitializeExpression)
                                     .WithValue(customBuilderConstructorInitializeExpression ?? (argumentType == null ? "{0} = new {2}Builder(source.{0});" : "{0} = new " + argumentType + "(source.{0});"))
            );

        public ClassPropertyBuilder ConvertCollectionPropertyToBuilderOnBuider(string? argumentType = null,
                                                                               string? customBuilderConstructorInitializeExpression = null)
            => ConvertCollectionOnBuilderToEnumerable().AddMetadata
            (
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderArgumentType)
                                     .WithValue(argumentType ?? "System.Collections.Generic.IEnumerable<{1}Builder>"),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderMethodParameterExpression)
                                     .WithValue("{0}.Select(x => x.Build())"),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderConstructorInitializeExpression)
                                     .WithValue(customBuilderConstructorInitializeExpression ?? (argumentType == null ? "{4}{0}.AddRange(source.{0}.Select(x => new {3}Builder(x)));" : "{4}{0}.AddRange(source.{0}.Select(x => new " + argumentType.GetGenericArguments() + "(x)));"))
            );
        ...
        public ClassPropertyBuilder AddBuilderOverload(string methodNameTemplate,
            ...
            => AddMetadata
            (
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderWithOverloadMethodName)
                                     .WithValue(methodNameTemplate),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderWithOverloadArgumentType)
                                     .WithValue(parameterTypeName),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderWithOverloadArgumentName)
                                     .WithValue(parameterNameTemplate),
                new MetadataBuilder().WithName(MetadataNames.CustomBuilderWithOverloadInitializeExpression)
                                     .WithValue(initializeExpression)
            );
        ...
        public ClassPropertyBuilder SetDefaultValueForBuilderClassConstructor(object defaultValue)
            => AddMetadata(new MetadataBuilder().WithName(MetadataNames.CustomImmutableDefaultValue)
                                                .WithValue(defaultValue));

        public ClassPropertyBuilder SetBuilderWithExpression(string expression)
            => AddMetadata(new MetadataBuilder().WithName(MetadataNames.CustomBuilderWithExpression)
