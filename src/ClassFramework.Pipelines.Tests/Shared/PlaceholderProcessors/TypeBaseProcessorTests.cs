...
public class TypeBaseProcessorTests : TestBase<TypeBaseProcessor>
{
    public class Process : TypeBaseProcessorTests
    {
        ...
        public void Returns_Continue_On_Unknown_Value()
        {
            // Arrange
            var sut = CreateSut();
            var context = new PipelineContext<ClassBuilder, BuilderContext>(Model, new BuilderContext(CreateModel(), new Pipelines.Builder.PipelineSettings(), CultureInfo.InvariantCulture));
